...
bool SessionUser::Set(SOCKET sock, SOCKADDR_IN address)
{
	this->m_Sock = sock;
...
int SessionUser::SendMsg(short type, char* msg)
{
	UPACKET packet;
...
int SessionUser::SendMsg(UPACKET& packet)
{
	m_wsaSendBuffer.buf = (char*)&packet;
	m_wsaSendBuffer.len = packet.ph.len;
...
void SessionUser::DispatchRead(DWORD dwTrans)
{
	...
	if (m_iReadPos >= PACKET_HEADER_SIZE)
	{
		...
		if (packet->ph.len < m_iReadPos)
		{
			do {
				...
				UPACKET* packet = (UPACKET*)&m_szDataBuffer[m_iPacketPos];
			} while (packet->ph.len <= m_iPacketPos);
		}
	}
}

void SessionUser::DispatchSend(DWORD dwTrans)
{
}
